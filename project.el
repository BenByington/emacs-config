...
(defun proj-generic-build-dirs-new (proj arch debug)
    (setq baseDir proj-root)
    (setq relDir (concat baseDir "/build/" proj "/" arch "/x86_64"))
    (cond ( debug       (setq build-type "/Debug"))
          ( (not debug) (setq build-type "/Release"))
	  (t                                      (setq build-type "invalid-build-arch"))
    )
    (message "Sanity %s %s" relDir build-type)
    (when (file-directory-p (concat relDir build-type)) 
          (setq return (concat relDir build-type)))
)
...
(defun proj-hwmongo()     (interactive)(setq proj-root "~/pa-mongo/hw-mongo/")  (setq proj-build-system "ninja") (setq proj-subproj "HwMongo")(fset 'proj-build-dir 'proj-hwmongo-build-dir) (defvaralias 'run-hist 'hw-mongo-hist)   (proj-run-args (car run-hist)) (proj-update-parse-proj))
